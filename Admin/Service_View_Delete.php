                            ...
                            <table class="table">
                                ...
                                if (mysql_num_rows($selectRes) == 0) {
                                    ...
                                    while ($row = mysql_fetch_assoc($selectRes)) {
                                        $gandhi = $row['Service_name'];
                                        $id = $row['Service_id'];
                                        if ($mit % 2 == 1) {
                                            echo "<tr class=\"active\"><th>{$row['Service_id']}</th><td align=\"center\">{$row['Service_name']}</td><td align=\"center\"><input type='button' value='DELETE' onclick='mfunction(this)' data-name='$gandhi' data-id='$id' class=\"btn btn-default\"></td></tr>\n";
                                        } else {
                                            echo "<tr><th>{$row['Service_id']}</th><td align=\"center\">{$row['Service_name']}</td><td align=\"center\"><input type='button' value='DELETE' onclick='mfunction(this)' data-name='$gandhi'  data-id='$id' class=\"btn btn-default\"></td></tr>\n";
                                        }
                                        $mit++;
                                    }
                                }
