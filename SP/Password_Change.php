...
<head>
    ...
    if(isset($_POST['Submit'])){
        ...
        $mit=0;
        $select="SELECT `Password` FROM `sprovider_master` WHERE Emp_id='{$_SESSION['User_id']}'";
        $sresult=mysql_query($select,$con);
        ...
        elseif($NPassword != $RPassword){
            ...
            $query="UPDATE `sprovider_master` SET Password='$NPassword' WHERE Emp_id='{$_SESSION['User_id']}'";
            $res=mysql_query($query,$con);
            if($res){
                echo "<script>alert('Pasword is Successfully Changed! You have to Log In again.');document.location='../Logout.php';</script>";
            }
            else{
